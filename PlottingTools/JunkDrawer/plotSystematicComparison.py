...
parser.add_argument('-d', '--datacard', type=str, default="", help="Datacard from which to extract the list of nuisances, if not specified then use list specified in macro.")
...
#samples = ["WH_hbb"]
#samples = ["WH_hbb","ZH_hbb","TT","Wj0b","Wj1b","Wj2b","VVHF","VVLF","Zj0b","Zj1b","Zj2b","s_Top"]
#samples = ["ggZH_hbb","ZH_hbb","TT","Wj0b","Wj1b","Wj2b","VVHF","VVLF","Zj0b","Zj1b","Zj2b","s_Top"]
samples = ["EWKWJets","TT","Wjets","Zjets","s_Top","VV"]
...
#systematics =  ["CMS_vhbb_puWeight","CMS_vhbb_eff_e_Wln_13TeV"]
systematics = ["CMS_vhbb_tune_vh","CMS_vhbb_WH_hbbModel_Wln_13TeV"]
#systematics = ['CMS_vhbb_scale_j_PileUpDataMC_13TeV', 'CMS_vhbb_scale_j_PileUpPtRef_13TeV', 'CMS_vhbb_scale_j_PileUpPtBB_13TeV', 'CMS_vhbb_scale_j_PileUpPtEC1_13TeV', 'CMS_vhbb_scale_j_PileUpPtEC2_13TeV', 'CMS_vhbb_scale_j_RelativeJEREC1_13TeV', 'CMS_vhbb_scale_j_RelativeFSR_13TeV', 'CMS_vhbb_scale_j_RelativeStatFSR_13TeV', 'CMS_vhbb_scale_j_RelativeStatEC_13TeV', 'CMS_vhbb_scale_j_RelativePtBB_13TeV', 'CMS_vhbb_scale_j_RelativePtEC1_13TeV', 'CMS_vhbb_scale_j_RelativePtEC2_13TeV', 'CMS_vhbb_scale_j_AbsoluteScale_13TeV', 'CMS_vhbb_scale_j_AbsoluteMPFBias_13TeV', 'CMS_vhbb_scale_j_AbsoluteStat_13TeV', 'CMS_vhbb_scale_j_SinglePionECAL_13TeV', 'CMS_vhbb_scale_j_SinglePionHCAL_13TeV', 'CMS_vhbb_scale_j_Fragmentation_13TeV', 'CMS_vhbb_scale_j_FlavorQCD_13TeV']
#systematics = ["CMS_vhbb_ptwweights2","CMS_vhbb_ptwweights5","CMS_vhbb_ptwweights10","CMS_vhbb_ptwweights15","CMS_vhbb_ptwweights20","CMS_vhbb_ptwweights30","CMS_vhbb_ptwweights50"]
#systematics = ["CMS_vhbb_ptwweights"]
#systematics = ["CMS_vhbb_LHE_weights_scale_muF_Wj0b","CMS_vhbb_LHE_weights_scale_muF_Zj0b","CMS_vhbb_bTagWeightHFStats2_high_central"]
...
if args.datacard != "":
    systematics = []
    datacard_file = open(args.datacard,"r")
    for line in datacard_file:
        if line[0] == '#': continue
        if line.find("shape       ")!=-1:
            name = line.split()[0]
            systematics.append(name)

print systematics

channel = args.channel
#ifile = ROOT.TFile(args.inputfile.replace("WmnHighPt",channel), "r")
ifile = ROOT.TFile(args.inputfile+"/hists_"+channel+".root", "r")
#c = ROOT.TCanvas("c","c")
for systematic in systematics:
    for sample in samples:
        ...
        try:
            ...
            print "BDT_%s_%s_%sUp" % (channel,sample,systematic)
            continue
        ...
        yield_up = hUp.Integral()
        yield_down = hDown.Integral()
        yield_nom = hnominal.Integral()
        ...
        l = ROOT.TLegend(0.65, 0.8, 0.85, 0.65)
        l.SetLineWidth(2)
        l.SetBorderSize(0)
        l.SetFillColor(0)
        l.SetFillStyle(4000)
        l.SetTextFont(62)
        l.SetTextSize(0.035)
        l.AddEntry(hnominal,"nominal (%f)" % yield_nom,"PL")
        l.AddEntry(hUp, "up (%f)" % yield_up,"PL")
        l.AddEntry(hDown, "down (%f)" % yield_down,"PL")
        ...
        ratioU.SetStats(0)
        #ratioU.GetYaxis().SetRangeUser(0.8,1.2)
        ratioU.GetYaxis().SetRangeUser(0.90,1.10)
        #ratioU.GetYaxis().SetRangeUser(0.5,1.5)
        ratioU.GetYaxis().SetNdivisions(502,0)
        ratioD.SetStats(0)
        #ratioD.GetYaxis().SetRangeUser(0.85,1.15)
        ratioD.GetYaxis().SetRangeUser(0.7,1.3)
