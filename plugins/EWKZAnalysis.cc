...
EWKZAnalysis::EWKZAnalysis(){
    if(debug>10) std::cout<<"Constructing EWKZAnalysis"<<std::endl;
}

// remove any 'new'
EWKZAnalysis::~EWKZAnalysis(){
    if(debug>10) std::cout<<"Deconstructing EWKZAnalysis"<<std::endl;
}

//probably want to do bdtSetup here
void EWKZAnalysis::InitAnalysis(){
...
bool EWKZAnalysis::Preselection(){
    //return true; // for the moment don't impose any preselection
    bool sel=true;
    //if (*in["evt"] != 422177307) return false;
    ...
    *f["met_pt_JECDown_ratio"] = *f["met_shifted_JetEnDown_pt"] / *f["met_pt"] ;
    ...
    for (int i=0; i<*in["nJet"]; i++) {
        if (int(*f["doReg"]) == 0) {
            ...
            //std::cout<<"Now jet_pt_reg is "<<f["Jet_pt"][i]<<std::endl;
        }
    ...
    for (int i=0; i < *in["nselLeptons"]; i++) {
        if (f["selLeptons_pt"][i] > *f["LepPtPresel"] && fabs(f["selLeptons_eta"][i]) < 2.5 ) {
            nPreselLep++;
        }
    }
    ...
    if ((*f["Vtype"]!=0 && *f["Vtype"]!=1) || nPreselJets < 2 || nPreselLep < 2) return false;

    TLorentzVector J1,J2,Hjj;
    J1.SetPtEtaPhiM(f["Jet_pt"][indJet1], f["Jet_eta"][indJet1], f["Jet_phi"][indJet1], f["Jet_mass"][indJet1] );
    J2.SetPtEtaPhiM(f["Jet_pt"][indJet2], f["Jet_eta"][indJet2], f["Jet_phi"][indJet2], f["Jet_mass"][indJet2] );
    Hjj = J1 + J2;
    float Mjj = Hjj.M();
...
bool EWKZAnalysis::Analyze(){
    ...
    *in["JetInd4"] = -1;
    ...
    TLorentzVector J1,J2,Hjj;
    J1.SetPtEtaPhiM(f["Jet_pt"][*in["JetInd1"]], f["Jet_eta"][*in["JetInd1"]], f["Jet_phi"][*in["JetInd1"]], f["Jet_mass"][*in["JetInd1"]] );
    J2.SetPtEtaPhiM(f["Jet_pt"][*in["JetInd2"]], f["Jet_eta"][*in["JetInd2"]], f["Jet_phi"][*in["JetInd2"]], f["Jet_mass"][*in["JetInd2"]] );
    Hjj = J1 + J2;
    *f["Mjj"] = Hjj.M();
    if (debug > 10000) {
    ...
    *in["lepInd"] = *in["lepInd1"];
    ...
    if (*in["isWmunu"] == 1) {
        ...
        if (*in["sampleIndex"] == 0) {
            // data event
            rochesterCorr1 = rc->kScaleDT(in["selLeptons_charge"][*in["lepInd1"]], f["selLeptons_pt"][*in["lepInd1"]], f["selLeptons_eta"][*in["lepInd1"]], f["selLeptons_phi"][*in["lepInd1"]], 0, 0);
            rochesterCorr2 = rc->kScaleDT(in["selLeptons_charge"][*in["lepInd2"]], f["selLeptons_pt"][*in["lepInd2"]], f["selLeptons_eta"][*in["lepInd2"]], f["selLeptons_phi"][*in["lepInd2"]], 0, 0);
        }
        else {
            ...
            double u3 = rand->Rndm();
            double u4 = rand->Rndm();
            rochesterCorr1 = rc->kScaleAndSmearMC(in["selLeptons_charge"][*in["lepInd1"]], f["selLeptons_pt"][*in["lepInd1"]], f["selLeptons_eta"][*in["lepInd1"]], f["selLeptons_phi"][*in["lepInd1"]],  in["selLeptons_trackerLayers"][*in["lepInd1"]], u1, u2, 0, 0);
            rochesterCorr2 = rc->kScaleAndSmearMC(in["selLeptons_charge"][*in["lepInd2"]], f["selLeptons_pt"][*in["lepInd2"]], f["selLeptons_eta"][*in["lepInd2"]], f["selLeptons_phi"][*in["lepInd2"]],  in["selLeptons_trackerLayers"][*in["lepInd2"]], u3, u4, 0, 0);
        }
        f["selLeptons_pt"][*in["lepInd1"]] = f["selLeptons_pt"][*in["lepInd1"]] * rochesterCorr1;
        f["selLeptons_pt"][*in["lepInd2"]] = f["selLeptons_pt"][*in["lepInd2"]] * rochesterCorr2;
    }
    *f["rochesterCorr1"] = rochesterCorr1;
    *f["rochesterCorr2"] = rochesterCorr2;
    ...
    *f["selLeptons_eta_0"] = f["selLeptons_eta"][*in["lepInd"]];
    ...
    TLorentzVector LL_vec = Lep1 + Lep2;
    *f["V_mass"] = LL_vec.M();
    if (fabs(*f["V_mass"] - 91) > 15) return false;
    ...
    *f["Jet2_qgl"] = f["Jet_qgl"][*in["JetInd2"]];
    ...
    if (*in["sampleIndex"]!=0) {
        if (*in["isWmunu"] == 1) {
                ...
                *f["Lep_SF"] = *f["Lep_SF"] *  (( (20.1/36.4) * f["SF_MuIDTightBCDEF"][*in["lepInd2"]] + (16.3/36.4) * f["SF_MuIDTightGH"][*in["lepInd2"]]) * ( (20.1/36.4) * f["SF_MuIsoTightBCDEF"][*in["lepInd2"]] + (16.3/36.4) * f["SF_MuIsoTightGH"][*in["lepInd2"]] ) * ( (20.1/36.4) * f["SF_MuTrackerBCDEF"][*in["lepInd2"]] + (16.3/36.4) * f["SF_MuTrackerGH"][*in["lepInd2"]]) );
               ...
               *f["Lep_SF"] = *f["Lep_SF"] * ( f["SF_ElIdIso"][*in["lepInd2"]] *  f["SF_egammaEffi_tracker"][*in["lepInd2"]]);
...
void EWKZAnalysis::TermAnalysis(){
...
bool EWKZAnalysis::ElectronSelection(){
...
bool EWKZAnalysis::MuonSelection(){
    ...
    *in["muInd2"] = -1;
    ...
    for (int i=0; i<*in["nselLeptons"]; i++) {
        if(fabs(in["selLeptons_pdgId"][i])==13 
            ...
            else if (*in["muInd1"]>-1 && f["selLeptons_pt"][i] > muMax2Pt && (in["selLeptons_charge"][i]*in["selLeptons_charge"][*in["muInd1"]]) < 0) {
...
float EWKZAnalysis::ptWeightQCD(int nGenVbosons, float lheHT, int GenVbosons_pdgId){
  float SF = 1.;
...
// from https://twiki.cern.ch/twiki/bin/view/CMS/VHiggsBBCodeUtils#V_X_QCD_and_EWK_corrections
float EWKZAnalysis::ptWeightEWK(int nGenVbosons,float GenVbosons_pt,int VtypeSim,int GenVbosons_pdgId){
  float SF = 1.;
  if (nGenVbosons ==1)
    {
...
double EWKZAnalysis::LOtoNLOWeightBjetSplitEtabb(double etabb, int njets){
...
void EWKZAnalysis::smearJets(float JERScale) {
    for (int i=0; i<*in["nJet"]; i++) {
...
float EWKZAnalysis::evaluateRegression(int i) {
...
float EWKZAnalysis::getQGLCorr_q(float x) {
    return (-0.666978*x*x*x + 0.929524*x*x -0.255505*x + 0.981581);
}

float EWKZAnalysis::getQGLCorr_g(float x) {
...
float EWKZAnalysis::getQGLCorr(float partonFlav, float eta, float qgl) {
    float corr = 1.0;
...
TLorentzVector EWKZAnalysis::getNu4Momentum(const TLorentzVector& TLepton, const TLorentzVector& TMET)
{
