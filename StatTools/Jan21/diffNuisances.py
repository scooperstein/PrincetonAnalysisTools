...
parser.add_option("--ptol", "--plot-tolerance", dest="ptol", default=0.0, type="float", help="Plot only nuisances with theta/dtheta > ptol, default 0.")
...
if options.plotfile:
    ...
    canvas_nuis.SetBottomMargin(0.2)
    canvas_nuis.SetTopMargin(0.2)
    if (options.ptol > 0.): 
        nNuisCleaned = 0
        threshold = options.ptol # theta / dTheta must be >= threshold 
        for ibin in range(1,hist_fit_b.GetNbinsX()+1):
            ...
            if ( (abs(theta/dtheta) > threshold) or (dtheta < (1-threshold))):
                nNuisCleaned += 1
        hist_fit_b_cleaned  = ROOT.TH1F("prefit_fit_b_cleaned"   ,"B-only fit Nuisances;;#theta ",nNuisCleaned,0,nNuisCleaned)
        hist_fit_s_cleaned  = ROOT.TH1F("prefit_fit_s_cleaned"   ,"S+B fit Nuisances;;#theta ",nNuisCleaned,0,nNuisCleaned)
        hist_prefit_cleaned = ROOT.TH1F("prefit_nuisancs_cleaned","Prefit Nuisances    ;;#theta ",nNuisCleaned,0,nNuisCleaned)
        nSet = 0
        for ibin in range(1,hist_fit_b.GetNbinsX()+1):
            ...
            #if ( (abs(theta_b/dtheta_b) > threshold)):
            if ( (abs(theta_b/dtheta_b) > threshold) or (dtheta_b < (1-threshold)) ):
                nSet += 1
                hist_fit_b_cleaned.SetBinContent(nSet,theta_b)
                hist_fit_b_cleaned.SetBinError(nSet,dtheta_b)
                hist_fit_b_cleaned.GetXaxis().SetBinLabel(nSet,label_b)
                hist_fit_s_cleaned.SetBinContent(nSet,theta_s)
                hist_fit_s_cleaned.SetBinError(nSet,dtheta_s)
                hist_fit_s_cleaned.GetXaxis().SetBinLabel(nSet,label_s)
                hist_prefit_cleaned.SetBinContent(nSet,hist_prefit.GetBinContent(ibin))
                hist_prefit_cleaned.SetBinError(nSet,hist_prefit.GetBinError(ibin))
                hist_prefit_cleaned.GetXaxis().SetBinLabel(nSet,hist_prefit.GetXaxis().GetBinLabel(ibin))
        hist_fit_b = hist_fit_b_cleaned
        hist_fit_s = hist_fit_s_cleaned
        hist_prefit = hist_prefit_cleaned
