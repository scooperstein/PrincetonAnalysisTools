...
inline int RocRes::getBin(double x, const int NN, const double *b) const{
    for(int i=0; i<NN; ++i) if(x<b[i+1]) return i;
    return NN-1;
}

inline RocRes::RocRes(){
    reset();
}

inline void RocRes::reset(){
    NETA=1;
    NTRK=1;
    NMIN=1;
...
inline int RocRes::getEtaBin(double feta) const{
    return getBin(feta,NETA,BETA);
}

inline int RocRes::getNBinDT(double v, int H) const{
    return getBin(v,NTRK,dtrk[H]);
}

inline int RocRes::getNBinMC(double v, int H) const{
    return getBin(v,NTRK,ntrk[H]);
}

inline void RocRes::dumpParams(){
    using namespace std;
...
inline void RocRes::init(std::string filename){
...
inline double RocRes::Sigma(double pt, int H, int F) const{
    double dpt=pt-45;
    return rmsA[H][F] + rmsB[H][F]*dpt + rmsC[H][F]*dpt*dpt;
}

inline double RocRes::getUrnd(int H, int F, double w) const{
    return ntrk[H][F]+(ntrk[H][F+1]-ntrk[H][F])*w; 
}

inline double RocRes::kSpread(double gpt, double rpt, double eta, int n, double w) const{
...
inline double RocRes::kSmear(double pt, double eta, TYPE type, double v, double u) const{
    int H = getBin(fabs(eta), NETA, BETA);
...
inline double RocRes::kSmear(double pt, double eta, TYPE type, double w, double u, int n) const{
    int H = getBin(fabs(eta), NETA, BETA);
    int F = n>NMIN ? n-NMIN : 0;
...
inline double RocRes::kExtra(double pt, double eta, int n, double u, double w) const{
    int H = getBin(fabs(eta), NETA, BETA);
    int F = n>NMIN ? n-NMIN : 0;
...
inline int RocOne::getBin(double x, const int NN, const double *b) const{
    for(int i=0; i<NN; ++i) if(x<b[i+1]) return i;
    return NN-1;
}

inline int RocOne::getBin(double x, const int nmax, const double xmin, const double dx) const{
    int ibin=(x-xmin)/dx;
    if(ibin<0) return 0; 
...
inline RocOne::RocOne(){
    reset();
}

inline RocOne::RocOne(std::string filename, int iTYPE, int iSYS, int iMEM){
    init(filename, iTYPE, iSYS, iMEM);
}


inline bool RocOne::checkSYS(int iSYS, int iMEM, int kSYS, int kMEM){
    if(iSYS==0 && iMEM==0)	      return true;
    if(iSYS==kSYS && iMEM==kMEM)      return true;
    return false;
}

inline bool RocOne::checkTIGHT(int iTYPE, int iSYS, int iMEM, int kTYPE, int kSYS, int kMEM){
...
inline void RocOne::init(std::string filename, int iTYPE, int iSYS, int iMEM){
...
inline double RocOne::kScaleDT(int Q, double pt, double eta, double phi) const{
...
inline double RocOne::kScaleMC(int Q, double pt, double eta, double phi, double kSMR) const{
    int H=getBin(eta, NETA, BETA);
    int F=getBin(phi, NPHI, MPHI, DPHI);
    double m=M[MC][H][F];
    double a=A[MC][H][F];
...
inline double RocOne::kScaleAndSmearMC(int Q, double pt, double eta, double phi, int n, double u, double w) const{
    double k=kScaleMC(Q, pt, eta, phi);
    return k*RR.kExtra(k*pt, eta, n, u, w);
}


inline double RocOne::kScaleFromGenMC(int Q, double pt, double eta, double phi, int n, double gt, double w) const{
    double k=kScaleMC(Q, pt, eta, phi);
    return k*RR.kSpread(gt, k*pt, eta, n, w);
}


inline double RocOne::kGenSmear(double pt, double eta, double v, double u, RocRes::TYPE TT) const{
...
inline RoccoR::RoccoR(){}

inline RoccoR::RoccoR(std::string dirname){
    init(dirname);
}
...
inline double RoccoR::kGenSmear(double pt, double eta, double v, double u, RocRes::TYPE TT, int s, int m) const{
    return RC[s][m].kGenSmear(pt, eta, v, u, TT);
}

inline double RoccoR::kScaleDT(int Q, double pt, double eta, double phi, int s, int m) const{
    return RC[s][m].kScaleDT(Q, pt, eta, phi);
}

inline double RoccoR::kScaleAndSmearMC(int Q, double pt, double eta, double phi, int n, double u, double w, int s, int m) const{
    return RC[s][m].kScaleAndSmearMC(Q, pt, eta, phi, n, u, w);
}

inline double RoccoR::kScaleFromGenMC(int Q, double pt, double eta, double phi, int n, double gt, double w, int s, int m) const{
