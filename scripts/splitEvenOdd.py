...
if not doVV:
    ...
    itree_even_sig = itree.CopyTree("Pass_nominal==1 && evt%2==0 && sampleIndex==-12501")
    ofile_even_sig.cd()
    ...
    itree_odd_sig = itree.CopyTree("Pass_nominal==1 && evt%2==1 && sampleIndex==-12501")
    ofile_odd_sig.cd()
    itree_odd_sig.Write()
    ...
    itree_even_sig = itree.CopyTree("Pass_nominal==1 && evt%2==0 && (sampleIndex>=3500&&sampleIndex<=3702)")
    ofile_even_sig.cd()
    itree_even_sig.Write()

    print "copying even bkg tree..."
    ofile_even_bkg = ROOT.TFile("%s/ofile_even_bkgvv.root" % ofiledir,"RECREATE")
    itree_even_bkg = itree.CopyTree("Pass_nominal==1 && evt%2==0 && (sampleIndex<3500||sampleIndex>3702)")
    ofile_even_bkg.cd()
    itree_even_bkg.Write()

    print "copying vv odd tree..."
    ofile_odd_sig = ROOT.TFile("%s/ofile_odd_vv.root" % ofiledir,"RECREATE")
    itree_odd_sig = itree.CopyTree("Pass_nominal==1 && evt%2==1 && (sampleIndex>=3500&&sampleIndex<=3702)")
    ofile_odd_sig.cd()
    itree_odd_sig.Write()

    print "copying odd bkg tree..."
    ofile_odd_bkg = ROOT.TFile("%s/ofile_odd_bkgvv.root" % ofiledir,"RECREATE")
    itree_odd_bkg = itree.CopyTree("Pass_nominal==1 && evt%2==1 && (sampleIndex<3500||sampleIndex>3702)")
    ofile_odd_bkg.cd()
    itree_odd_bkg.Write()
