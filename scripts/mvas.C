   ...
   while ((key = (TKey*)next())) {
      ...
      while ((titkey = (TKey*)keyIt())) {
         ...
         if (htype == CompareType) {
            ...
            // perform K-S test
            // perform KG test on sensitive bins, we don't care so much about the low stuff
            //sigOv->GetXaxis()->SetRangeUser(-0.6,1.0);
            //bgdOv->GetXaxis()->SetRangeUser(-0.6,1.0);
            //sig->GetXaxis()->SetRangeUser(-0.6,1.0);
            //bgd->GetXaxis()->SetRangeUser(-0.6,1.0);
            for (int ibin=0; ibin<sigOv->GetNbinsX(); ibin++) {
                sigOv->SetBinContent(ibin, fabs(sigOv->GetBinContent(ibin)));
                bgdOv->SetBinContent(ibin, fabs(bgdOv->GetBinContent(ibin)));
                sig->SetBinContent(ibin, fabs(sig->GetBinContent(ibin)));
                bgd->SetBinContent(ibin, fabs(bgd->GetBinContent(ibin)));
            }
            cout << "--- Perform Kolmogorov-Smirnov tests" << endl;
            Double_t kolS = sig->KolmogorovTest( sigOv, "X" );
            Double_t kolB = bgd->KolmogorovTest( bgdOv, "X" );
            cout << "--- Goodness of signal (background) consistency: " << kolS << " (" << kolB << ")" << endl;
            //sigOv->GetXaxis()->SetRangeUser(-1.0,1.0);
            //bgdOv->GetXaxis()->SetRangeUser(-1.0,1.0);
